...
public record Player(String name, String fullName,
                     LocalDate birthDate, int age, double heightCm, double weightKg,
    ...
    private static final String PERSON_ATTRIBUTE_DELIMITER = ";";
    private static final DateTimeFormatter DATE_FORMAT = DateTimeFormatter.ofPattern("M/d/yyyy");

    public static Player of(String line) {
        String[] tokens = line.split(PERSON_ATTRIBUTE_DELIMITER);

        String name = tokens[NAME];
        String fullName = tokens[FULL_NAME];
        LocalDate birthDate = LocalDate.parse(tokens[BIRTH_DATE], DATE_FORMAT);
        ...
        Foot preferedFoot = Foot.of(tokens[PREFERRED_FOOT]);

        return new Player(name, fullName, birthDate, age,
                heightCm, weightKg, positions, nationality,
                overallRating, potential, valueEuro, wageEuro, preferedFoot);
    }
